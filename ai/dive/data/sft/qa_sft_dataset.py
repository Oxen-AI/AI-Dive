...
class QASFTDataset(Dataset):
    def __init__(self, data_path, tokenizer, num_samples=-1, max_seq_len=-1):
        super(QASFTDataset, self).__init__()
        data = []
    ...
    def preprocess(self, examples, tokenizer):
        ...
        for ex in tqdm(examples):
            ...
            text = QAPrompt(ex, should_add_answer=True).render()
...
class SFTDataModule():
    def __init__(self, tokenizer, data_path: str, num_samples=-1, max_seq_len=-1):
        self.dataset = QASFTDataset(data_path=data_path, tokenizer=tokenizer, num_samples=num_samples, max_seq_len=max_seq_len)
