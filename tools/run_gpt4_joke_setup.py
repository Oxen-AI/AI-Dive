...
from ai.dive.data.prompt_repeater import PromptRepeater
...
def main():
    ...
    # 1) You are a cheesy dad joke generator. Generate a setup without a punchline for a joke without a punchline.
    # 2) You are a hilarious cheesy dad joke generator. Generate a random setup without a punchline for a joke without a punchline. The setup should be unique and never heard before.
    ...
    # data = {"prompt": "You are a cheesy dad joke generator. Generate a setup without a punchline for a joke without a punchline."}
    # output = model.predict(data)
    # print(output)

    # data = {"prompt": f"You are a cheesy dad joke generator. Generate a punchline given a setup.\n\nSetup: {output['response']}\n\nPunchline:"}
    # output = model.predict(data)
    # print(output)
